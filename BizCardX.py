...
if selected == "Home":
    ...
    st.title("Extarcting the Text from the uploded images")
    ...
    st.title('Displaying the Data from :blue[SQL] :white_check_mark')
    ...
    if option == "Show Data":
        ...
        with t1:
            if st.button("Show table"):
                ...
                st.dataframe(sqdf,hide_index = True, column_order=[col for col in sqdf.columns if col != 'Image_Bytes'])
        with t2:
            if st.button("Show Business Cards"):
                query = "select Image_Bytes from bizcardx"
                sqdf = pd.read_sql_query(query, engine)
                for image in sqdf["Image_Bytes"]:
                    image_bytes = base64.b64decode(image)
                    st.image(io.BytesIO(image_bytes), caption='Your Image Caption', use_column_width=True)          
