...
def upload_image(image_bytes):
    ...
    #Encoding the bytes to insert into database
...
#Extracting the text from image
    ...
    # Inserting the extracted text to Database
    ...
    if option == "Show Data":
        ...
        #To display the Extracted data from Database in Table Format
        with t1:
        ...
        #To display the Extracted images from Database
        with t2:
            if st.button("Show Business Cards"):
                ...
                for companyname,image in sqdf.to_records(index=False).tolist():
                    ...
                    #Decoding the extracted image back to Bytes to display
                    image_bytes = base64.b64decode(image)
                    st.image(io.BytesIO(image_bytes), caption=f'{companyname}')

    #To edit the data and update in Database
    ...
    #To delete the selected data and update in Database
        ...
        def fetch_data():
            ...
            df = pd.read_sql_query(query, engine)
            return df
        st.title('Deleting the selected rows and updating the :blue[Database]')
        with st.expander("Show table", expanded=True):
            df = fetch_data()
